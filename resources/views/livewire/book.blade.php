    ...
    <div x-data="{
        ...
        <aside class="w-full sm:w-1/3 md:w-1/4 px-2">
            <div class="sticky top-0 bg-white rounded-xl w-full">
                ...
                <p class="mt-2 mb-3 text-sm leading-6 text-gray-900">Published:
                    {{ $book->book_year_published > 0 ? $book->book_year_published : '----' }}</p>
                @if (!Auth::guest())
        ...
        <main role="main" class="w-full sm:w-2/3 md:w-3/4 pt-1 px-2">
            ...
            <p class="text-gray-600">By:
                <a class="text-xl" wire:navigate
                    href="{{ route('author', $book->author->author_slug) }}">{{ $book->author->getFullName() }}</a>
            </p>
